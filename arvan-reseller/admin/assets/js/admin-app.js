(function () {
	...
	const pageMeta = {
		...
		settlements: ['تسویه داخلی', 'گزارش حسابداری شبیه‌سازی‌شده؛ بدون ادعای پرداخت خارجی'],
	...
	async function renderDashboard() {
		...
		setContent(
			...
			'<div class="ar-stack"><article class="ar-card"><div class="ar-card-head"><h2>توزیع وضعیت سرویس</h2></div><div class="ar-donut-wrap"><div class="ar-donut"><span class="ar-donut__value"><strong>' + ui.persianDigits(resources.length) + '</strong>کل سرویس‌ها</span></div><div class="ar-legend"><span><i style="--legend-color:var(--ar-color-success)"></i>فعال ' + ui.persianDigits(active) + '</span><span><i style="--legend-color:var(--ar-color-warning)"></i>تعلیق ' + ui.persianDigits(suspended) + '</span><span><i style="--legend-color:var(--ar-color-danger)"></i>خطا ' + ui.persianDigits(resources.filter((r) => r.status === 'error').length) + '</span></div></div></article>' +
	...
	async function renderPayments() {
		...
		try {
			...
			const rows = filtered.map((payment) => ['<code dir="ltr">' + ui.escape(payment.payment_reference) + '</code>', '<code dir="ltr">#' + ui.escape(payment.id) + '</code>', ui.money(payment.amount, payment.currency), ui.status(payment.status), ui.escape(payment.provider), ui.date(payment.created_at), payment.status === 'completed' ? '<button class="ar-button ar-button--secondary ar-button--small" type="button" data-ar-refund="' + ui.escape(payment.payment_reference) + '">بازپرداخت</button>' : '—']);
			setContent(ui.pageHead(pageMeta.payments[0], pageMeta.payments[1], '<span class="ar-env ar-env--' + ui.escape(runtime.settings.mode) + '"><span></span>' + (runtime.settings.mode === 'mock' ? 'Mock' : 'Live') + '</span>') + '<article class="ar-card">' + toolbar({ placeholder: 'مرجع پرداخت یا مشتری', filter }) + table(['مرجع', 'شناسه', 'مبلغ', 'وضعیت', 'درگاه', 'زمان', 'عملیات'], rows, 'پرداختی یافت نشد') + '</article>');
	...
	async function renderOrders() {
		...
		try {
			...
			const rows = filtered.map((order) => ['<code dir="ltr">' + ui.escape(order.order_reference) + '</code>', '<code dir="ltr">#' + ui.escape(order.customer_id) + '</code>', 'Cloud Server', ui.status(order.status), '<code dir="ltr">' + ui.escape(order.resource_id || '—') + '</code>', ui.escape(order.region), ui.date(order.created_at)]);
			setContent(ui.pageHead(pageMeta.orders[0], pageMeta.orders[1]) + '<article class="ar-card">' + toolbar({ placeholder: 'مرجع، مشتری یا شناسه منبع' }) + table(['سفارش', 'مشتری', 'محصول', 'وضعیت', 'Resource ID', 'منطقه', 'ثبت'], rows, 'سفارشی یافت نشد') + '</article>');
	...
	async function renderResources() {
		...
		try {
			...
			setContent(ui.pageHead(pageMeta.resources[0], pageMeta.resources[1], '<button class="ar-button" type="button" data-ar-action="run-reconciliation">' + ui.icon('refresh') + 'بازیابی نگاشت‌ها</button>') + '<div class="ar-alert ar-alert--warning">' + ui.icon('info') + '<div><strong>مالکیت مشتری در این پاسخ REST ارائه نمی‌شود</strong><p>رابط فقط فیلدهای امن موجود را نمایش می‌دهد؛ برای نمایش مالکیت به تکمیل قرارداد backend نیاز است.</p></div></div><article class="ar-card" style="margin-top:16px">' + toolbar({ placeholder: 'Resource ID، منطقه یا وضعیت' }) + table(['Resource ID', 'منطقه', 'وضعیت محلی', 'وضعیت راه‌دور', 'نرخ ساعتی', 'آخرین همگام‌سازی'], rows, 'سروری یافت نشد') + '</article>');
	...
	async function renderSettlements() {
		...
		try {
			...
			setContent(ui.pageHead(pageMeta.settlements[0], pageMeta.settlements[1]) + '<div class="ar-alert ar-alert--warning">' + ui.icon('info') + '<div><strong>SIMULATED / INTERNAL ACCOUNTING</strong><p>این رکوردها خلاصه حسابداری داخلی و شبیه‌سازی‌شده‌اند و نشان‌دهنده پرداخت یا تسویه خارجی ArvanCloud نیستند.</p></div></div><article class="ar-card" style="margin-top:16px">' + table(['مرجع', 'وضعیت', 'شروع دوره', 'پایان دوره', 'هزینه پایه', 'دریافت مشتری', 'سهم فروشنده'], tableRows, 'تسویه داخلی ثبت نشده است') + '</article>');
	...
	async function renderHealth() {
		...
		'<section class="ar-layout-main"><article class="ar-card"><div class="ar-card-head"><h2>زمان‌بندی‌ها</h2><span class="ar-env ar-env--' + ui.escape(health.mode) + '"><span></span>' + ui.escape(health.mode) + '</span></div><dl class="ar-summary-list"><div><dt>همگام‌سازی مصرف</dt><dd>' + (schedules.usage ? ui.date(Number(schedules.usage) * 1000) : 'زمان‌بندی نشده') + '</dd></div><div><dt>بازیابی منابع</dt><dd>' + (schedules.reconciliation ? ui.date(Number(schedules.reconciliation) * 1000) : 'زمان‌بندی نشده') + '</dd></div><div><dt>تسویه داخلی</dt><dd>' + (schedules.settlement ? ui.date(Number(schedules.settlement) * 1000) : 'زمان‌بندی نشده') + '</dd></div></dl></article><article class="ar-card"><div class="ar-card-head"><h2>محدودیت عملیاتی</h2></div><div class="ar-alert ar-alert--warning">' + ui.icon('warning') + '<div><strong>WP-Cron وابسته به ترافیک است</strong><p>در محیط تولید، اجرای زمان‌بندی‌شده wp-cron.php باید پایش شود. خاموش‌کردن Cron داخلی تنها پس از تأیید زمان‌بند بیرونی امن است.</p></div></div></article></section>');
	...
	async function renderSettings() {
		...
		try {
			...
			const panels = [
				'<section><span class="ar-eyebrow">ArvanCloud Commerce</span><h2>کنسول فروش Cloud Server را آماده کنید</h2><p>این راهنما تنظیمات واقعی backend را ذخیره می‌کند. ابتدا Mock را کامل کنید و Live را تنها پس از ثبت امن Machine User فعال کنید.</p><div class="ar-alert ar-alert--success">' + ui.icon('shield') + '<div><strong>Backend معتبر و جدا از رابط است</strong><p>کیف پول، دفترکل، سفارش، صورتحساب و امنیت بدون بازنویسی باقی می‌مانند.</p></div></div></section>',
				...
				'<section><h2>حالت اجرای سرویس</h2><div class="ar-mode-cards"><label class="ar-select-card"><input type="radio" name="mode" value="mock"' + (settings.mode !== 'live' ? ' checked' : '') + '><strong>Mock — پیشنهاد برای دمو</strong><small>قطعی، بدون شبکه و بدون هزینه واقعی</small></label><label class="ar-select-card"><input type="radio" name="mode" value="live"' + (settings.mode === 'live' ? ' checked' : '') + '><strong>Live — نیازمند کلید امن</strong><small>هیچ fallback پنهانی به Mock ندارد</small></label></div><div class="ar-form-grid" style="margin-top:20px">' + settingField('region', 'منطقه پیش‌فرض', settings.region, 'text', '', 'dir="ltr" pattern="[a-z0-9][a-z0-9-]{1,39}"') + settingField('availability_zone', 'Availability Zone', settings.availability_zone, 'text', '', 'dir="ltr"') + '</div></section>',
			...
			setContent(ui.pageHead(pageMeta[page][0], pageMeta[page][1], '<span class="ar-env ar-env--' + ui.escape(settings.mode) + '"><span></span>' + ui.escape(settings.mode) + '</span>') + '<form id="ar-settings-form" class="ar-card"><div class="ar-wizard"><nav class="ar-wizard-nav" aria-label="مراحل راه‌اندازی">' + stepNav + '</nav><div class="ar-wizard-panel">' + panels.map((panel, index) => panel.replace('<section', '<section class="' + (index === state.wizardStep ? 'is-active' : '') + '"')).join('') + actions + '</div></div></form>');
