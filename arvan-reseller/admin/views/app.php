	...
	<div class="ar-app-frame">
		...
		<div class="ar-workspace">
			<header class="ar-topbar">
				...
				<div class="ar-topbar__actions"><label class="ar-theme-picker"><span class="ar-icon" data-icon="theme" aria-hidden="true"></span><span class="screen-reader-text"><?php esc_html_e( 'پوسته نمایش', 'arvan-reseller' ); ?></span><select data-ar-theme-select aria-label="<?php esc_attr_e( 'پوسته نمایش', 'arvan-reseller' ); ?>"><option value="system"><?php esc_html_e( 'سیستم', 'arvan-reseller' ); ?></option><option value="light"><?php esc_html_e( 'روشن', 'arvan-reseller' ); ?></option><option value="dark"><?php esc_html_e( 'تیره', 'arvan-reseller' ); ?></option></select></label><span class="ar-env ar-env--<?php echo esc_attr( $mode ); ?>"><span></span><?php echo esc_html( $mode_label ); ?></span><span class="ar-avatar" aria-hidden="true"><?php $display_name = wp_get_current_user()->display_name; echo esc_html( function_exists( 'mb_substr' ) ? mb_substr( $display_name, 0, 1 ) : substr( $display_name, 0, 1 ) ); ?></span></div>
			</header>
