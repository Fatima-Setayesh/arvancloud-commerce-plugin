(function () {
	...
	async function dashboard() {
		...
		const usageTotal = usage.reduce((sum, row) => sum + numeric(row.total_charge), 0).toFixed(4);
		...
		setContent(pageHead('<button class="ar-button ar-button--secondary" type="button" data-ar-route="wallet">' + ui.icon('wallet') + 'شارژ کیف پول</button><button class="ar-button ar-button--accent" type="button" data-ar-route="create-server">' + ui.icon('plus') + 'ساخت سرور</button>') +
			...
			'<section class="ar-grid ar-grid--metrics ar-customer-metrics" style="margin-top:16px"><article class="ar-card ar-wallet-hero"><small>موجودی کیف پول</small><strong class="ar-wallet-balance">' + (wallet ? ui.money(wallet.balance, wallet.currency) : '—') + '</strong><button class="ar-button ar-button--accent" type="button" data-ar-action="open-topup">' + ui.icon('plus') + 'شارژ کیف پول</button></article>' + metric('سرویس‌های فعال', ui.persianDigits(active.length), 'server', ui.persianDigits(resources.length) + ' سرویس ثبت‌شده') + metric('مصرف دوره', ui.money(usageTotal, runtime.settings.currency), 'chart', ui.persianDigits(usage.length) + ' پنجره مصرف') + metric('اعلان‌های اخیر', ui.persianDigits(notifications.length), 'bell', 'آخرین رویدادهای دریافت‌شده') + '</section>' +
	...
	async function resourceDetail() {
		...
		try {
			...
			setContent(pageHead('<button class="ar-button ar-button--secondary" type="button" data-ar-route="services">بازگشت به سرویس‌ها</button>') +
				'<article class="ar-card ar-resource-hero"><div class="ar-resource-hero__identity"><span class="ar-resource-hero__icon">' + ui.icon('server') + '</span><div><span class="ar-eyebrow">سرور ابری</span><h2>' + ui.escape(resource.region || 'سرویس ابری') + '</h2><div class="ar-resource-id"><code dir="ltr">' + ui.escape(resource.resource_id) + '</code><button class="ar-icon-button" type="button" data-copy="' + ui.escape(resource.resource_id) + '" aria-label="کپی شناسه">' + ui.icon('copy') + '</button></div></div></div>' + ui.status(resource.status) + '</article>' +
				...
				'<section class="ar-resource-strip"><article><span class="ar-icon" data-icon="region"></span><small>منطقه</small><strong><code dir="ltr">' + ui.escape(resource.region || '—') + '</code></strong></article><article><span class="ar-icon" data-icon="server"></span><small>وضعیت راه‌دور</small><strong>' + ui.status(resource.remote_status) + '</strong></article><article><span class="ar-icon" data-icon="chart"></span><small>نرخ ساعتی</small><strong>' + ui.money(resource.hourly_price, runtime.settings.currency) + '</strong></article><article><span class="ar-icon" data-icon="receipt"></span><small>آخرین صورتحساب</small><strong>' + ui.date(resource.last_billed_at) + '</strong></article></section>' +
				'<section class="ar-layout-main"><div class="ar-stack"><article class="ar-card"><div class="ar-card-head"><div><h2>مصرف و هزینه سرویس</h2><p>مقادیر قطعی ثبت‌شده برای همین شناسه منبع</p></div><button class="ar-button ar-button--secondary ar-button--small" type="button" data-ar-route="billing">جزئیات مصرف</button></div>' + ui.lineChart(usageValues.length ? usageValues : [0]) + '</article><div class="ar-alert">' + ui.icon('info') + '<div><strong>نمایش بر پایه داده موجود</strong><p>تصویر، پلن و منابع سخت‌افزاری در پاسخ امن فعلی ارائه نمی‌شوند و در این صفحه حدس زده نشده‌اند.</p></div></div></div><aside class="ar-stack"><article class="ar-card ar-resource-wallet"><div class="ar-card-head"><h2>کیف پول</h2>' + ui.status(walletData.status) + '</div><strong class="ar-metric__value">' + ui.money(walletData.balance, walletData.currency) + '</strong><small>موجودی قابل استفاده حساب</small><button class="ar-button ar-button--accent ar-button--block" type="button" data-ar-action="open-topup">شارژ کیف پول</button></article><article class="ar-card"><div class="ar-card-head"><h2>چرخه عمر</h2></div>' + provisioningTimeline(resource.status, resource.resource_id) + '</article><article class="ar-card"><div class="ar-card-head"><h2>صورتحساب‌ها</h2></div><strong class="ar-metric__value">' + ui.persianDigits(invoices.length) + '</strong><small>صورتحساب‌های حساب مشتری</small><button class="ar-button ar-button--secondary ar-button--block" type="button" data-ar-route="billing" style="margin-top:16px">مشاهده صورتحساب</button></article></aside></section>');
		} catch (error) { setContent(pageHead() + ui.error(error, 'retry-route')); }
	}
	...
	async function configurator() {
		...
		try {
			...
			const actions = '<div class="ar-wizard-actions ar-config-actions"><button class="ar-button ar-button--secondary" type="button" data-ar-action="config-prev"' + (state.configStep === 0 ? ' disabled' : '') + '>مرحله قبل</button>' + (state.configStep < 4 ? '<button class="ar-button" type="button" data-ar-action="config-next"' + (canNext ? '' : ' disabled') + '>ادامه پیکربندی</button>' : '<button class="ar-button ar-button--accent" type="button" data-ar-action="create-order"' + (state.estimate ? '' : ' disabled') + '>' + ui.icon('server') + 'ثبت سفارش و ساخت سرور</button>') + '</div>';
	...
	function summaryPanel(catalog) {
		...
		return '<aside class="ar-card ar-config-summary"><div class="ar-card-head"><div><span class="ar-eyebrow">سفارش سرور ابری</span><h2>خلاصه سفارش</h2></div>' + ui.status('pending') + '</div><dl class="ar-summary-list"><div><dt>موقعیت</dt><dd><code dir="ltr">' + ui.escape(state.config.region || '—') + '</code></dd></div><div><dt>سیستم‌عامل</dt><dd>' + ui.escape(state.config.image && state.config.image.name || '—') + '</dd></div><div><dt>منابع</dt><dd>' + ui.escape(state.config.flavor && state.config.flavor.name || '—') + '</dd></div><div><dt>دیسک ریشه</dt><dd>' + ui.persianDigits(state.config.rootVolumeSizeGigaBytes) + ' گیگابایت</dd></div><div><dt>نام سرور</dt><dd><code dir="ltr">' + ui.escape(state.config.name || '—') + '</code></dd></div></dl><div class="ar-summary-wallet"><span class="ar-icon" data-icon="wallet" aria-hidden="true"></span><div><small>موجودی کیف پول</small><strong>' + ui.money(catalog.wallet.balance, catalog.wallet.currency) + '</strong></div></div>' + (state.estimate ? '<div class="ar-price-box"><small>برآورد معتبر ۲۴ ساعت</small><strong>' + ui.money(state.estimate.total_charge, state.estimate.currency) + '</strong></div>' : '') + (insufficient ? '<div class="ar-alert ar-alert--warning">' + ui.icon('warning') + '<div><strong>موجودی برای این برآورد کافی نیست</strong><p>پیش از ثبت سفارش، کیف پول را شارژ کنید.</p></div></div>' : '') + '<small class="ar-config-summary__note">قیمت قطعی از سرویس برآورد سمت سرور دریافت می‌شود.</small></aside>';
	}
