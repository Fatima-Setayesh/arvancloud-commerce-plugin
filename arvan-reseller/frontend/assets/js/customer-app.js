(function () {
	...
	const routeMeta = {
		dashboard: ['داشبورد', 'نمای کوتاه از کیف پول و سرویس‌ها'], services: ['سرویس‌ها', 'سرورهای ابری متعلق به حساب شما'],
		'create-server': ['ساخت سرور ابری', 'پیکربندی مرحله‌ای با برآورد معتبر سمت سرور'], wallet: ['کیف پول', 'شارژ آزمایشی و دفترکل تغییرناپذیر'],
		billing: ['مصرف و صورتحساب', 'پنجره‌های دقیق مصرف و صورت‌حساب‌ها'], orders: ['سفارش‌ها', 'رهگیری ساخت و شناسه منبع'],
		notifications: ['اعلان‌ها', 'هشدارهای موجودی و رویدادهای سرویس'], resource: ['جزئیات سرویس', 'وضعیت واقعی ثبت‌شده در سامانه']
	};
	...
	function title() { const meta = routeMeta[state.route] || routeMeta.dashboard; app.querySelector('#ar-customer-page-title').textContent = meta[0]; document.title = meta[0] + ' — فروش ابری آروان'; }
	...
	async function services() {
		...
		try {
			...
			setContent(pageHead('<button class="ar-button ar-button--accent" type="button" data-ar-route="create-server">' + ui.icon('plus') + 'ساخت سرور جدید</button>') + (resources.length ? '<section class="ar-grid ar-grid--3">' + resources.map(resourceCard).join('') + '</section>' : ui.empty('هنوز سرور ابری ندارید', 'منطقه، سیستم‌عامل و پلن را از کاتالوگ سرویس انتخاب کنید.', '<button class="ar-button ar-button--accent" type="button" data-ar-route="create-server">شروع پیکربندی</button>')));
	...
	function openTopup() {
		...
		modal.dialog.querySelector('#ar-topup-form').addEventListener('submit', async (event) => {
			...
			try {
				...
				api.completeOperation(operation); modal.close(); ui.toast('پرداخت آزمایشی ساخته شد؛ برای شارژ، آن را تأیید کنید.', 'success'); await wallet();
	...
	function regionPanel(catalog) { return '<div class="ar-card-head"><div><h2>منطقه سرویس</h2><p>منطقه از کاتالوگ سامانه دریافت شده است.</p></div></div><div class="ar-option-grid">' + catalog.regions.map((region) => { const key = region.name || region.id; return optionCard('region', key, key, [region.status || ''], state.config.region === key); }).join('') + '</div>'; }
	...
	function reviewPanel(catalog) { return '<div class="ar-card-head"><div><h2>بررسی و برآورد سمت سرور</h2><p>برآورد برای ۲۴ ساعت استفاده درخواست می‌شود؛ تخمین ماهانه ساخته نمی‌شود.</p></div></div>' + (state.estimate ? '<div class="ar-price-box"><small>برآورد معتبر ۲۴ ساعت</small><strong>' + ui.money(state.estimate.total_charge, state.estimate.currency) + '</strong><span>هزینه پایه ' + ui.money(state.estimate.base_cost, state.estimate.currency) + ' + سهم فروشنده ' + ui.money(state.estimate.reseller_share, state.estimate.currency) + '</span><small>دریافت‌شده در ' + ui.date(state.estimateAt) + ' · حالت ' + modeLabel(state.estimate.mode) + '</small></div>' : ui.loading('در حال دریافت برآورد معتبر...')) + '<div class="ar-alert ar-alert--warning">' + ui.icon('info') + '<div><strong>برآورد، صورتحساب نهایی نیست</strong><p>صورتحساب واقعی براساس پنجره‌های استفاده سامانه و نرخ ذخیره‌شده سفارش ثبت می‌شود.</p></div></div><label class="ar-check" style="margin-top:18px"><input type="checkbox" data-terms required><span>شرایط سرویس پیش‌پرداخت و محدودیت‌های سرور ابری را خوانده‌ام.</span></label><div class="ar-alert" style="margin-top:16px">' + ui.icon('wallet') + '<div><strong>موجودی فعلی</strong><p>' + ui.money(catalog.wallet.balance, catalog.wallet.currency) + ' — سامانه مانده بعد از سفارش را در برآورد ارائه نمی‌کند؛ مقداری حدس زده نمی‌شود.</p></div></div>'; }

	function summaryPanel(catalog) {
		return '<aside class="ar-card ar-config-summary"><div class="ar-card-head"><h2>خلاصه سفارش</h2>' + ui.status('pending') + '</div><dl class="ar-summary-list"><div><dt>منطقه</dt><dd><code dir="ltr">' + ui.escape(state.config.region || '—') + '</code></dd></div><div><dt>تصویر</dt><dd>' + ui.escape(state.config.image && state.config.image.name || '—') + '</dd></div><div><dt>پلن منابع</dt><dd>' + ui.escape(state.config.flavor && state.config.flavor.name || '—') + '</dd></div><div><dt>دیسک ریشه</dt><dd>' + ui.persianDigits(state.config.rootVolumeSizeGigaBytes) + ' گیگابایت</dd></div><div><dt>نام</dt><dd><code dir="ltr">' + ui.escape(state.config.name || '—') + '</code></dd></div></dl>' + (state.estimate ? '<div class="ar-price-box"><small>۲۴ ساعت</small><strong>' + ui.money(state.estimate.total_charge, state.estimate.currency) + '</strong></div>' : '') + '<small>قیمت قطعی از سرویس برآورد سمت سرور دریافت می‌شود.</small></aside>';
	}
	...
	async function createOrder(button) {
		...
		try {
			...
			setContent(pageHead('<button class="ar-button" type="button" data-ar-route="services">مشاهده سرویس</button>') + '<section class="ar-provision-banner">' + ui.icon('server') + '<div><span class="ar-status ar-status--success">سفارش ثبت شد</span><h2>سرور ابری در حالت آزمایشی آماده شد</h2><p>شناسه منبع با موفقیت دریافت و نگاشت محلی ثبت شد.</p><code dir="ltr">' + ui.escape(order.resource_id || 'در انتظار') + '</code></div></section><article class="ar-card" style="margin-top:16px"><h2>رهگیری فرایند ساخت</h2>' + provisioningTimeline(order.status, order.resource_id) + '</article>');
	...
	async function orders() {
		...
		try {
			...
			setContent(pageHead('<button class="ar-button ar-button--accent" type="button" data-ar-route="create-server">سفارش جدید</button>') + '<article class="ar-card ar-card--flush">' + table(['مرجع سفارش', 'وضعیت', 'منطقه', 'شناسه منبع', 'ثبت', 'جزئیات'], rows, 'سفارشی ثبت نشده است') + '</article>');
