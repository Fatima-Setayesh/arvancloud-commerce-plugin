(function () {
	...
	async function orders() {
		...
		try {
			...
			const rows = ordersData.map((order) => ['<code dir="ltr">' + ui.escape(order.order_reference) + '</code>', '<strong>' + ui.escape(order.configuration && order.configuration.name || 'سرور ابری') + '</strong><br><small><code dir="ltr">' + ui.escape(order.configuration && order.configuration.flavorId || '—') + '</code></small>', ui.status(order.status), order.quote && order.quote.total_charge ? ui.money(order.quote.total_charge, order.quote.currency) : '—', ui.escape(order.payment && order.payment.status || '—'), order.recovery_required ? ui.status('failed').replace('ناموفق', 'نیازمند بازیابی') : (order.failure_code ? '<code dir="ltr">' + ui.escape(order.failure_code) + '</code>' : '<code dir="ltr">' + ui.escape(order.resource_id || '—') + '</code>'), ui.date(order.created_at)]);
			setContent(pageHead('<button class="ar-button ar-button--accent" type="button" data-ar-route="create-server">سفارش جدید</button>') + '<article class="ar-card ar-card--flush">' + table(['مرجع سفارش', 'پیکربندی', 'وضعیت', 'برآورد ۲۴ساعته', 'پرداخت سفارش', 'منبع/بازیابی', 'ثبت'], rows, 'سفارشی ثبت نشده است') + '</article>');
