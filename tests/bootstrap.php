...
class Arvan_Test_Database extends Arvan_Reseller_Database {
	...
	public function get_resources_for_balance_policy( $id, $after_id = 0, $limit = 50 ) { return array_values( array_slice( array_filter( $this->resources, static function( $r ) use ( $id, $after_id ) { return (int) $r['customer_id'] === (int) $id && (int) $r['id'] > (int) $after_id && in_array( (string) $r['status'], array( 'provisioning', 'active', 'provisioned', 'suspended' ), true ); } ), 0, $limit ) ); }
